...
void TRA_switch::recv_handler (packet *p){
    ...
    if (p->type() == "TRA_ctrl_packet") { // the switch receives a packet from the controller
        ...
        // change contant
        if(routingTable.find(p3->getHeader()->getDstID()) == routingTable.end()) {
            return; // not found
        }
        p3->getHeader()->setPreID ( getNodeID() );
        ...
        if(getNodeID() != p3->getHeader()->getDstID()) {
            send_handler(p3); // send package to next nodes
        }
    }
...
int main()
{
    int nodeSize, dstSize, linkSize, pairSize, simTime;
    cin >> nodeSize >> dstSize >> linkSize >> pairSize >> simTime;
    ...
    for(int i = 0; i < dstSize; i++) cin >> dstList[i].id >> dstList[i].broadcastTime;
    for(int i = 0; i < linkSize; i++) cin >> linkList[i].id >> linkList[i].node1 >> linkList[i].node2;
    for(int i = 0; i < pairSize; i++) cin >> flowList[i].id >> flowList[i].src >> flowList[i].dst >> flowList[i].size >> flowList[i].startTime;

    // // check input
    // cout << "-----------\nout:\n\nsimTime:" << simTime << endl;
    // for(int i = 0; i < dstSize; i++) cout << "dst id: " << dstList[i].id << "  bt:" << dstList[i].broadcastTime << endl;
    // for(int i = 0; i < linkSize; i++) cout << "link id: " << linkList[i].id << "  node: " << linkList[i].node1 << "  " << linkList[i].node2 << endl;
    // for(int i = 0; i < pairSize; i++) cout << "flow id: " << flowList[i].id << " src: " << flowList[i].src<< " dst: " << flowList[i].dst << " size: " << flowList[i].size << " startTime: " << flowList[i].startTime << endl;
    ...
    for (unsigned int id = 0; id < nodeSize; id ++){
    ...
    //     cout << "node " << n->getNodeID() << "'s nblist: ";
    //     for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
    //         cout << it->first << ", " ;
    //     }
    //     cout << endl;
    ...
    for(auto f: flowList) {
        data_packet_event(f.src, f.dst, f.startTime);
    }
    ...
    for(int id = 0; id < nodeSize; id++) {
        cout << id << endl;
        TRA_switch *tra = (TRA_switch*)node::id_to_node(id);
        for(auto t: tra->getRoutingTable()) {
