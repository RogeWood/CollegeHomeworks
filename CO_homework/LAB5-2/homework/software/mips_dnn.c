...
void read_ppm_file(int idxi)
{
    ...
    int dim0 = DIM[0];
	for (idxi = 0; idxi < dim0; idxi++)								// 讀取圖像像素值，將其轉換為浮點數並存儲在v_hidden數組中
	{
		fscanf(fp, "%d", &temp);
...
void parameter_init()
{
	...
	{
        int idxz_sub1;
        int dim_idxz = DIM[idxz];
        int dim_idxz_sub1 = DIM[idxz_sub1];
...
int main()
{
	...
	for (k = 0; k < NUM_OF_TEST; k++)
	{
		...
		for (idxz = 1; idxz < 3; idxz++)
		{
            int idxz_sub1;
            int dim_idxz = DIM[idxz];
            int dim_idxz_sub1_mul2 = DIM[idxz_sub1] * 2;

            for (idxj = 1; idxj <= dim_idxz_sub1_mul2; idxj = idxj + 2)
            {
                *(io + (DM_BASE_OFFSET / 4) + idxj + 1) = *(unsigned int *)&v_hidden[idxz_sub1][(idxj - 1) / 2];
            }
			...
			for (idxi = 0; idxi < DIM[idxz]; idxi++)
			{
				...
				// 計算加權總和
				*(io + (DM_BASE_OFFSET / 4)) = dim_idxz_sub1_mul2 * 4;
				for (idxj = 1; idxj <= dim_idxz_sub1_mul2; idxj = idxj + 2)
				{
					*(io + (DM_BASE_OFFSET / 4) + idxj) = *(unsigned int *)&w_hidden[idxz_sub1][idxi][(idxj - 1) / 2];				
					// *(io + (DM_BASE_OFFSET / 4) + idxj + 1) = *(unsigned int *)&v_hidden[idxz_sub1][(idxj - 1) / 2];
				}
				...
				*(io + (CPU_RSTN_OFFSET / 4)) = 0;
				...
				temp_f = *((float *)&(*(io + (DM_BASE_OFFSET / 4) + dim_idxz_sub1_mul2 + 1)));
